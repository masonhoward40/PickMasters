...
export function GroupsTab({ groups, refetchGroups }) {
  ...
  const fetchGroups = async () => {
    try {
      ...
      const response = await fetch(`/api/admin/groups/list?${params}`);
      if (response.ok) {
        const data = await response.json();
        setPaginatedGroups(data.groups);
  ...
  const handleEditGroup = (group) => {
    console.log("✏️ Edit group:", group);
  ...
  const handleGroupSaved = () => {
    console.log("✅ Group saved, refetching...");
    fetchGroups();
  };
  ...
  return (
    <div>
      ...
      <CreateGroupForm onGroupCreated={handleGroupSaved} />

      <div className="mt-6">
        {/* Status Tabs */}
        <div className="flex gap-2 mb-4">
          {["active", "completed", "deleted", "all"].map((s) => (
            <button
              key={s}
              onClick={() => {
                setStatus(s);
        ...
        <div className="bg-white dark:bg-[#1E1E1E] rounded-xl border border-[#E6E6E6] dark:border-[#333333] p-4 mb-4">
          ...
          <div className="flex items-center gap-2 mt-3">
            ...
            <div className="text-sm text-[#6F6F6F] dark:text-[#AAAAAA] font-inter">
              Showing {paginatedGroups.length} of {totalCount} groups
            </div>
          </div>
        </div>
